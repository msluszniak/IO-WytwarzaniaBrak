...
class _MapState extends State<MapPage> {
      ...
      body: FlutterMap(
        ...
        layers: [
          ...
          TappablePolylineLayerOptions(
            ...
            polylines: [
              TaggedPolyline(points: routingPointsToDraw, color: Colors.blue, strokeWidth: 6.0),
            ],
          ),
        ],
        children: <Widget>[
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        ...
                        showModalBottomSheet(
                            ...
                            builder: (BuildContext context) => FutureBuilder<List<Equipment>>(
                                future: dbManager.getJoined<Gym, Equipment>(gymList[index].id),
                                builder: (context, snapshot) {
                                  if (!snapshot.hasData) {
                                    return CircularProgressIndicator();
                                  } else {
                                    List<String> tags = snapshot.data!.map((e) => e.name).toList();
  ...
  void _showRoute() async {
    if (routingPointsToDraw.isEmpty) {
      ...
      if (workout != null) {
        ...
        middle = [
          for (Gym gym in workout.gymsToVisit) gym.getLatLng(),
        ];
        _drawRoute(
          start,
          finish,
  ...
  void _drawRoute(LatLng startingPoint, LatLng endingPoint, [List<LatLng> middlePoints = const []]) async {
    try {
      final route =
          await Route.Route.createRoute(startingPoint, endingPoint, middlePoints).timeout(Duration(seconds: 5));
